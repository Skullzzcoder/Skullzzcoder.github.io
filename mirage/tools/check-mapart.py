...
check("and every way out leaves a reason", paint.count("lastReason =") >= 5)
...
check("a map the client has never seen is named as such",
      "the client has no map" in paint and "Hold a real map" in paint)
...
print("FAILED: " + "; ".join(fails) if fails else
      ...
      "tall for one card; the hand-over cannot throw and always says why"
